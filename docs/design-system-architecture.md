...
| **LESS** | Octicons only: `frappe/public/css/octicons/octicons.less` |
| **PostCSS** | SASS compilation, Autoprefixer, RTL (rtlcss), cssnano (production) |
| **Bootstrap 4.6** | Base component framework; variables overridden in SCSS |
...
│  • bootinfo (desk_theme, assets)  • hooks (app_include_css/js)               │
...
│  bench build → yarn build → esbuild.js → SASS + PostCSS → dist/*.css         │
...
| **CSS framework** | Bootstrap 4 with custom overrides; Espresso tokens for design tokens |
...
| PostCSS | autoprefixer, rtlcss, cssnano | PostCSS plugin | Root |
...
| **LESS** | octicons.less | `frappe/public/css/octicons/octicons.less` | Octicons |

### Import Hierarchy (Desk)

```
desk.bundle.scss
├── fonts (fontawesome, octicons, inter)
├── frappe-charts, plyr, leaflet CSS
└── desk/index.scss
    ├── variables.scss (espresso + Bootstrap overrides)
    ├── common/mixins, global, icons, alert
    ├── ~bootstrap/scss/bootstrap
    ├── desk/global, typography
    ├── common/buttons, flex
...
| `--focus-default`, `--focus-blue` | _shadows.scss | Focus rings | Inputs, buttons |
...
| -------- | -------------- | ------- |
| `--padding-xs` … `--padding-2xl` | 5px … 40px | Spacing |
| `--margin-xs` … `--margin-2xl` | 5px … 40px | Spacing |
...
- **Bootstrap 4** base with variable overrides.
...
- Error: `--error-bg`, `--error-border` (desk/css_variables.scss).
...
| 2xl | `--shadow-2xl` | 0px 10px 24px | Popovers, dropdowns |
...
- **Mixin:** `@mixin flex($dis, $x, $y, $dir)` in common/mixins.scss.
- **Bootstrap:** `.d-flex`, `.justify-content-*`, `.align-items-*`.

### Spacing Utilities

- **Variables:** `--padding-xs` … `--padding-2xl`, `--margin-xs` … `--margin-2xl`.
- **Bootstrap:** `$spacers` in website/variables.scss.
...
4. `MutationObserver` in desk.js re-runs `set_theme()` when `data-theme-mode` changes.
...
| **Compilation** | `get_scss()` → Node script → `/files/website_theme/*.css` |
